...
void Utility::getCursorPos(double &x, double &y){
...
void Utility::GravMovement(glm::mat4& a_transform, float a_deltaTime, float a_speed, float floorPos,CollisionMap *cMap, const glm::vec3& a_up /* = glm::vec3(0,1,0) */) {
	GLFWwindow* window = glfwGetCurrentContext();
	...
	if (glfwGetKey(window,'W') == GLFW_PRESS){
		if (cMap!= nullptr){
			if (cMap->CheckMap(vTranslation.xyz,HitLocation)){
